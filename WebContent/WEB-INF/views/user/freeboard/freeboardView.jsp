	...
	<div class="panel panel-green">
		...
		<div class="panel-body pan">
			...
			<form action="/user/freeboard/updateBoardInfo.do"  id ="freeboardView"  role="form" method="post"> 
					...
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
	 								...
	 								<label for="inputName" class="control-label"  for="bo_nickname"> NickName</label>
										<div class="input-icon right">
											<i class="fa fa-user"></i> <input id="bo_nickname" name="bo_nickname" type="text"  class="form-control" />
										</div>
										
			                        <label for="inputPassword" class="control-label" for="bo_pwd"> Password</label>
			                             <div class="input-icon right">
			                                 <i class="fa fa-lock"></i><input id="bo_pwd" type="password" name="bo_pwd"  class="form-control"  disabled/>
			                             </div>
	         ...
	         <div class="form-group"> 
			   <label class="control-label col-sm-2" for="file01">FILE:</label>
								...
								<c:forEach items="${freeboardInfo.items}" var="fileitem" varStatus="stat">
									...
									<c:if test="${stat.last}">
										...
										<img src="/files/${fileitem.file_save_name}" alt="pic1"         
											onclick="javascript:location.href='/user/freeboard/freeboardFile.do?fileSEQ=${fileitem.file_seq }&bo_no=${fileitem.file_bo_no}';">
				...
				<div class="form-group">
						<label for="inputSubject" class="control-label col-sm-2" for="bo_title"> Title</label>
						<div class="input-icon right">
							<i class="fa fa-tag"></i> <input id="bo_title" type="text"  name="bo_title" placeholder="" class="form-control" />
						</div>

						<label for="inputMessage" class="control-label col-sm-2" for="bo_content"> Content</label>
						<textarea rows="10" id ="bo_content" class="form-control" name="bo_content"></textarea>
  ...
  $('#bo_content').val('${freeboardInfo.bo_content}');	
	...
	$('#deletebtn').click(function(){
		var flag = redirectFlag();
    	 ...
    	 var bo_no= $('#bo_no').val();
    	 if(flag){
    	$(location).attr('href', '/user/freeboard/deleteBoardInfo.do?bo_no='+ bo_no);
    	 }else{
    ...
    $('#replybtn').click(function(){
    	...
    	var url = '${pageContext.request.contextPath}/user/freeboard/freeboardReplyBoardForm.do';

...
<div class="panel panel-black" style="background:#fff;">
        ...
        <div class="panel-body pan">
            <form action="${pageContext.request.contextPath}/user/news/updateNewsInfo.do" method="post" id="newsView">
                    ...
                    <div class="col-md-4">
                        ...
                        <input id="news_no" name="news_no" type="hidden" placeholder="" class="form-control" />
                            ...
                            <div class="input-icon right">
                                <input id="news_title" name="news_title" type="text" placeholder="" class="form-control" />
                                </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <label for="news_mem_id" class="control-label">
                                작성자</label>
                            <div class="input-icon right">
                                <input readonly id="news_mem_name" name="news_mem_name" type="text" placeholder="" class="form-control" />
                                </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <label for="news_reg_date" class="control-label">
                                작성일</label>
                            <div class="input-icon right">
                                <input readonly id="news_reg_date" name="news_reg_date" type="text" placeholder="" class="form-control" />
                                </div>
                        </div>
                    </div>
                </div>
...
// 	});
	...
	$('#newsListBtn').click(function(){
		$(location).attr('href', '/user/news/newsList.do');
	});
	...
	$('#deleteBtn').click(function(){
		...
		if(flag){
			$(location).attr('href', '${pageContext.request.contextPath}/user/news/deleteNewsInfo.do?news_no=${newsInfo.news_no}');
		}else{

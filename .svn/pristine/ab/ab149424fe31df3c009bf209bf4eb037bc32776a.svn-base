...
<script>
	$(function(){
		...
		$('#qnaListBtn').click(function(){
			$(location).attr('href', '/admin/qna/qnaList.do');
		});
		...
		$('#deleteBtn').click(function(){
			...
			if(flag){
				$(location).attr('href', '${pageContext.request.contextPath}/admin/qna/deleteQnaInfo.do?qa_no=${qnaInfo.qa_no}');
			}else{
		...
		$('#qnaView').submit(function(){
			...
			if(flag){
				$(location).attr('action', '${pageContext.request.contextPath}/admin/qna/updateQnaInfo.do');
			}else{
				alertPRT('해당 게시글은 작성자만 삭제할 수 있습니다.');
			}
		});
		...
		$('#reply').click(function(){
			url = '/admin/qna/qnaReplyBoardForm.do';
			qa_title = $('#qa_title').val();
			
			$(location).attr('href', url + '?rnum=${qnaInfo.rnum}&qa_title=' + qa_title + '&qa_group=${qnaInfo.qa_group}&qa_seq=${qnaInfo.qa_seq}&qa_depth=${qnaInfo.qa_depth}');
		});
...
<!-- 				</div>  -->
            ...
            <div class="form-actions text-right pal">
                ...
                <button type="button" class="btn btn-primary" id="reply">답글</button>
